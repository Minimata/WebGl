...
class Cylinder{
	...
	init(){
		...
		//close first circle
		this.indices.push(0,this.division+offset,offset);
		//define indexes, draw top circle
		this.indices.push(1);
		for(var i=this.division+offset+1;i<this.division*2+offset+1;i++)
		{
			this.indices.push(1,i,i+1);
		}
		//close second circle
		this.indices.push(1,this.division+offset+1,this.division*2+offset+1);
		//draw surface between circles
		for(var i = offset; i<this.division+1;i++){
